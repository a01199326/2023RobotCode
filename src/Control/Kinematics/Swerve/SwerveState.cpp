...
wpi::array<frc::SwerveModuleState, 4> SwerveState::toWPIVel() const {
...
wpi::array<frc::SwerveModulePosition, 4> SwerveState::toWPIPose() const {
    std::array<frc::SwerveModulePosition, 4> states;
    for (size_t i = 0; i < wheels.size(); i++) {
        const auto &wheel(wheels[i]);
        states[i].distance = units::meter_t(wheel->wheelPosition);
        states[i].angle = frc::Rotation2d(units::angle::radian_t(wheel->wheelAngle));
    }
...
std::ostream &operator<<(std::ostream &os, const SwerveState &value) {
	os << "fl: " << value.topLeft << std::endl;
	os << "fr: " << value.topRight << std::endl;
	os << "bl: " << value.backLeft << std::endl;
	os << "br: " << value.backRight;
	return os;
}