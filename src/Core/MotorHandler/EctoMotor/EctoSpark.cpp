...
EctoSpark::EctoSpark(int id, const std::string &name, bool brushed, bool doTests) : EctoMotor(id, name,
    ...
    absoluteEncoder = std::make_unique<rev::SparkMaxAbsoluteEncoder>(sparkBase->GetAbsoluteEncoder(rev::SparkMaxAbsoluteEncoder::Type::kDutyCycle));
    ...
    pidControllerBase->SetPositionPIDWrappingEnabled(false);
}
...
bool EctoSpark::isSensorInverted() const {
	...
	switch (feedbackMode) {
        ...
        case MotorFeedbackMode::AbsoluteEncoder:
            return absoluteEncoder->GetInverted();
		default:
			return false;
	}
}
...
void EctoSpark::setPIDConfig(const PIDConfig &pidConfig, int profileSlot) {
	...
	pidControllerBase->SetOutputRange(-1.0, 1.0);
}

void EctoSpark::enableContinuousInput(double minVal, double maxVal) {
    pidControllerBase->SetPositionPIDWrappingEnabled(true);
    pidControllerBase->SetPositionPIDWrappingMinInput(minVal);
    pidControllerBase->SetPositionPIDWrappingMaxInput(maxVal);
}
...
void EctoSpark::invertSensor(bool state) {
    ...
    absoluteEncoder->SetInverted(state);
}
...
void EctoSpark::setVelocitySetpoint(double velocity) {
    double vel = (velocity / (2 * M_PI)) * 60;
	pidControllerBase->SetReference(vel,
...
void EctoSpark::setAbsolutePositionConversionFactor(double conversionFactor) {
    absoluteEncoder->SetPositionConversionFactor(conversionFactor);
}

void EctoSpark::setAbsoluteVelocityConversionFactor(double conversionFactor) {
    absoluteEncoder->SetVelocityConversionFactor(conversionFactor);
}

void EctoSpark::setPositionSetpoint(double position) {
    double pos = EctoMath::wrapAngle(position + analogOffset);
    pidControllerBase->SetReference(pos, rev::CANSparkMaxLowLevel::ControlType::kPosition);
}
...
double EctoSpark::getAbsPosition() const {
    std::lock_guard<std::mutex> lg(mutex);
...
double EctoSpark::getAbsVelocity() const {
    std::lock_guard<std::mutex> lg(mutex);
...
void EctoSpark::setAbsoluteAsClosedLoopSource() {
    pidControllerBase->SetFeedbackDevice(*absoluteEncoder);
}
...
void EctoSpark::setAbsoluteSensorOffset(double offset) {
    EctoSpark::absoluteOffset = offset;
}
...
void EctoSpark::followMotor(const EctoMotor &masterMotor, bool inverted) {
	...
	switch (masterMotor.getMotorType()) {
		...
		case EctoMotorType::SparkMax:
            //invert to make isInverted more logical
...
void EctoSpark::prioritizeUpdateRate() {
	sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus0, 20);
	sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus1, 20);
    sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus2, 20);
}

void EctoSpark::prioritizeAbsUpdateRate() {
    sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus0, 20);
    sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus1, 20);
    sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus2, 20);
    sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus4, 20);
    sparkBase->SetPeriodicFramePeriod(rev::CANSparkMaxLowLevel::PeriodicFrame::kStatus5, 20);
}
...
bool EctoSpark::getSensorErrorFault() {
    return sparkBase->GetFault(rev::CANSparkMax::FaultID::kSensorFault);
}