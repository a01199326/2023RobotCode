...
void SystemHandler::robotInit() {
	for (auto &systemData: systems) {
		log->info("SetRobotPosition Init: {}", systemData.subsystem->getName());
...
std::string TimingData::getData() const {
	...
	data << fmt::format("SetRobotPosition Init: {}sec Update Update Average: {}sec", initRobotTime, updateRobotTime) << std::endl;
